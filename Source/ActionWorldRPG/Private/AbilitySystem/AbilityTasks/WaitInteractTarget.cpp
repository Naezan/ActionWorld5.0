...
#include "AbilitySystem/AbilityTasks/WaitInteractTarget.h"
#include "Character/Player/PlayerBase.h"
#include "Interfaces/InteractInterface.h"
#include "DrawDebugHelpers.h"
#include "TimerManager.h"

UWaitInteractTarget::UWaitInteractTarget(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	bTraceAffectsAimPitch = true;
}

UWaitInteractTarget* UWaitInteractTarget::WaitForInteractableTarget(UGameplayAbility* OwningAbility, FName TaskInstanceName, FCollisionProfileName TraceProfile, float MaxRange, float TimerPeriod, bool bShowDebug)
{
	UWaitInteractTarget* MyObj = NewAbilityTask<UWaitInteractTarget>(OwningAbility, TaskInstanceName);		//Register for task list here, providing a given FName as a key
	MyObj->TraceProfile = TraceProfile;
	MyObj->MaxRange = MaxRange;
	MyObj->TimerPeriod = TimerPeriod;
	MyObj->bShowDebug = bShowDebug;

	APlayerBase* Hero = Cast<APlayerBase>(OwningAbility->GetCurrentActorInfo()->AvatarActor);
...
void UWaitInteractTarget::Activate()
{
	UWorld* World = GetWorld();
	World->GetTimerManager().SetTimer(TraceTimerHandle, this, &UWaitInteractTarget::PerformTrace, TimerPeriod, true);

}

void UWaitInteractTarget::OnDestroy(bool AbilityEnded)
{
	UWorld* World = GetWorld();
...
void UWaitInteractTarget::LineTrace(FHitResult& OutHitResult, const UWorld* World, const FVector& Start, const FVector& End, FName ProfileName, const FCollisionQueryParams Params, bool bLookForInteractableActor) const
{
	check(World);
...
void UWaitInteractTarget::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params, const FVector& TraceStart, FVector& OutTraceEnd, bool bIgnorePitch) const
{
...
bool UWaitInteractTarget::ClipCameraRayToAbilityRange(FVector CameraLocation, FVector CameraDirection, FVector AbilityCenter, float AbilityRange, FVector& ClippedPosition) const
{
...
void UWaitInteractTarget::PerformTrace()
{
	...
	APlayerBase* Hero = Cast<APlayerBase>(SourceActor);
...
FGameplayAbilityTargetDataHandle UWaitInteractTarget::MakeTargetData(const FHitResult& HitResult) const
{
